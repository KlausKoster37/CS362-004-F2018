...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    //Refactored
    playAdventurer(drawntreasure, state, currentPlayer, temphand, z, handPos);
    return 0;
...
void playAdventurer(int drawntreasure, struct gameState *state, int currentPlayer, int *temphand, int z, int handPos)
{
  int cardDrawn;
  ...
  //discard card from hand - fixed by kosterkl  -  Added discard card to adventurer to increase played card count
  discardCard(handPos, currentPlayer, state, 0);
}
...
{
    //+3 Cards
  for (int i = 0; i < 3; i++) //Bug: draw 5 cards instead of 3.. fixed by kosterkl - draw only 3 cards
	{
	  drawCard(currentPlayer, state);
	}
